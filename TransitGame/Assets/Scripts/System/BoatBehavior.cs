	...
	void Update () {
		...
		if (can_move)
		{
			...
			if (Input.GetMouseButtonDown(0))
			{
				...
				{
					if ((hit.collider.gameObject.CompareTag("Wave") || hit.collider.gameObject.CompareTag("Waypoint")) && !hit.collider.gameObject.CompareTag("Boat"))
					{
						if (Vector2.Distance(new Vector2(hit.point.x, hit.point.z), new Vector2(transform.position.x, transform.position.z)) > 10f)
						{
							move_position = new Vector2(hit.point.x, hit.point.z);
							Instantiate(Resources.Load<GameObject>("Anchor"), new Vector3(move_position.x, 3, move_position.y), new Quaternion());
							spd_lerp = 0f;
							angle_lerp = 0f;
							moving = true;
						}
